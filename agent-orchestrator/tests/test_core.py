...
@pytest.mark.parametrize(
    "mensaje,esperado",
    [
        ("¿Cuál es el horario de atención?", "soporte"),
        ("¿Cuánto cuesta el plan?", "soporte"),
        ("Quiero cancelar y pedir reembolso", "soporte"),
        ("Necesito agendar una reunión", "agenda"),
        ("Mueve mi cita del jueves", "agenda"),
        ("Resume el informe trimestral", "investigacion"),
    ],
)
def test_preguntas_cortas_no_escalan_por_ser_concisas(mensaje, esperado):
    """Regresión: una consulta breve y clara debe enrutarse, no escalar.

    El scoring se normaliza por cobertura de la consulta, así que una
    pregunta de pocas palabras no queda penalizada frente a una larga.
    """
    state = _orchestrator().run(mensaje)
    routing = [s for s in state.steps if s.type is StepType.ROUTING][0]
    assert routing.metadata["agent_id"] == esperado
    assert state.status is RunStatus.COMPLETED


def test_termino_suelto_en_frase_ajena_si_escala():
    """Contrapeso del test anterior: una coincidencia aislada sigue escalando."""
    state = _orchestrator().run("Mi perro se llama Pluto y ladra mucho")
